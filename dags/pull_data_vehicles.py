...
GCS_FILE_PATH ="crashes/vehicles.csv"
import pandas as pd

columns=['crash_unit_id', 'crash_record_id', 'crash_date', 'unit_no','unit_type', 'vehicle_id', 'make', 'model']
...
def extract_data(offset=0, chunk_size=100000):
    ...
    while True:
        ...
        try:
            ...
            df["crash_unit_id"] = df["crash_unit_id"].astype(int)
...
with DAG(
    ...
    upload_py_file_to_gcs = LocalFilesystemToGCSOperator(
        ...
        src="./dags/vehicles_data_spark.py",  # Local CSV file
